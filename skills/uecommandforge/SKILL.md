---
name: uecommandforge
description: Use when the agent needs to automate Unreal Engine work through UECommandForge, including Blueprint, asset, C++ reflection, DataTable, config, project-rule validation, or Result JSON inspection tasks. Use before running Unreal automation, choosing UECommandForge wrappers, checking installation manifests, or handling requests that might otherwise tempt direct Unreal Python access.
---
...
```text
<AGENT_HOME>/UECommandForge/specs/agent/unreal-automation-agents.md
```

저장소에서 개발 중이면 repo의 `specs/agent/unreal-automation-agents.md`를 사용한다. 이 skill은 실행 절차의 요약이며, 임시 산출물 위치와 새 automation 구현 요구사항은 상세 규칙을 기준으로 한다.

## 설치 상태 확인

1. 에이전트 홈(AGENT_HOME)을 정한다. 대상 에이전트에 맞춰 ~/.codex, ~/.claude, 또는 ~/.gemini를 사용한다.
2. 다음 파일을 확인한다.
   - `<AGENT_HOME>/UECommandForge/uecommandforge-installed.json`
   - `<AGENT_HOME>/UECommandForge/uecommandforge.env`
   - `<AGENT_HOME>/UECommandForge/uecommandforge.env.sh`
...
```text
<AGENT_HOME>/UECommandForge/tools/ue
```
