...
- installer smoke는 기본적으로 `--run-commandlet-check false`를 사용한다. UE commandlet 기반 post-install 검증은 실제 UE 실행 환경에서 별도 실행해야 한다.
...
- `--run-commandlet-check true` 기반 UE post-install 검증
- Phase 8 DataAsset/DataTable/Config 제품화 Task 진행
- release version `0.8.0` 승격 및 `CHANGELOG.md` 작성 여부 결정
