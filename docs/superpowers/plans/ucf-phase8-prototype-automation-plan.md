...
- [x] **Step 1: `PrototypeAutomationCommandlet` 작성**

asset policy, C++ policy, data policy를 한 번에 실행하는 통합 검증 Commandlet을 추가한다.

- [x] **Step 2: `tools/ue/validate_project_rules.sh/.bat` 추가**
...
- 각 child commandlet report를 별도 JSON으로 남기고, 통합 JSON report의 `steps`, `validation.suite_count`, `validation.failed_suite_count`, `issues`에 집계한다.
- `-Markdown` 인자가 없으면 통합 JSON report와 같은 basename의 Markdown report를 생성한다. Markdown에는 suite별 commandlet, PASS/FAIL, issue/error 수, child report 경로를 표로 기록한다.
- `tools/ue/validate_project_rules.sh/.bat` wrapper와 `tools/test/smoke/prototype_automation.sh/.bat` 스모크를 추가했다.
- 검증: RED는 누락 header compile failure로 확인했고, `./tools/ue/build_plugin.sh`, sample plugin in-place 빌드, `./tools/test/automation/run.sh`, `./tools/test/smoke/windows_command_wrappers.sh`, `UE_COMMANDLET_TIMEOUT=120 ./tools/test/smoke/prototype_automation.sh`를 통과했다. 자동화 테스트는 52개 PASS, 0개 FAIL, 0개 SKIP이다.

## Task 5: 문서와 README 업데이트

- [x] **Step 1: README 명령 표면 갱신**
...
- 2026-05-29: README 명령 표면을 Phase 8 기준으로 갱신했다. asset/C++/data/config 제품화 wrapper와 `validate_project_rules.sh/.bat`, `prototype_automation.sh/.bat` 스모크, PrototypeAutomation Markdown report 안내를 추가했다.

- [x] **Step 2: 마스터 플랜 갱신**

Phase 8 상태, 검증 결과, 남은 리스크를 기록한다.

- [x] **Step 3: 리뷰/eval 리포트 작성**
...
- 2026-05-29: 마스터 플랜에 Phase 8 제품화 구현 완료 상태, 최종 검증 결과, Windows 실기 검증 잔여 리스크를 기록했다. `ucf-phase8-review-eval-report.md`는 제품화/eval 범위와 PrototypeAutomation 리뷰 반영 사항까지 포함하도록 갱신했다.
...
진행 상태:
- 2026-05-28: `tools/release/write_manifest.sh`가 release manifest의 `version`, `engine_version`, `release_channel`, `package_type`, file lists, checksums, `install_commands`, `post_install_checks`, `known_limits`를 생성한다.
...
- [x] **Step 8: 배포 전 게이트 정의**

릴리즈 전 필수 게이트:
- `git diff --check` 통과
- `tools/test/smoke/release_version_policy.sh` 통과
- `tools/ue/build_plugin.sh` 통과
- sample plugin in-place build 통과
- `tools/test/automation/run.sh` 통과
- `tools/test/smoke/windows_command_wrappers.sh` 통과
- Phase 8 제품화 smoke 통과:
  - `tools/test/smoke/asset_policy.sh`
  - `tools/test/smoke/asset_change_apply_rollback.sh`
  - `tools/test/smoke/cpp_generation.sh`
  - `tools/test/smoke/cpp_reflection_policy.sh`
...
- Windows build와 `.bat` 실기 검증 통과. macOS-only 릴리즈 후보에서는 `tools/test/smoke/windows_release_validation_report.sh`로 `windows_host_required` 제한 리포트를 생성하고 release notes에 명시한다.
- 모든 release ZIP은 sibling `checksums.txt`를 생성하고 `tools/release/verify_release_package.sh <zip> <checksums.txt>`를 통과한다.
- secret scan은 hardcoded token/API key/password 패턴이 source, tools, specs, docs release surface에 없는지 확인한다. false positive는 release/eval report에 사유를 남긴다.
- README install/command surface, `CHANGELOG.md`, `release-notes.md`, `validation-report.json`, review/eval report가 같은 version/channel을 가리킨다.

2026-05-29 완료:
- README에 배포 게이트 섹션을 추가하고, macOS 로컬 게이트와 Windows 제한 리포트 기준을 문서화했다.
- 배포 전 게이트는 build/test/smoke/package/install/update/uninstall/checksum/docs/eval을 모두 포함하는 필수 목록으로 고정한다.

- [x] **Step 9: 배포 후 검증 정의**

배포 후 확인:
- GitHub Release artifact가 다운로드 가능하다.
- artifact 목록에는 최소 `Plugin`, `Tools`, `Source`, `checksums.txt`, `release-notes.md`, `install.md`, `validation-report.json`이 포함된다.
- 다운로드한 각 ZIP은 배포된 `checksums.txt`와 `tools/release/verify_release_package.sh`로 검증한다.
- 새 clone 또는 깨끗한 sample project에서 `install-uecommandforge.sh` 또는 `tools/release/install_local.sh` 설치가 성공한다.
- 설치 후 `Hello`, `CreateAIFlow`, `ValidateProjectRules`가 Result JSON `ok: true`를 남긴다.
- update는 backup을 생성하고, uninstall은 기본 모드에서 Codex tools/specs를 보존하며, full uninstall 옵션에서 지정한 항목을 제거한다.
- release notes에는 Windows 실기 검증 여부, known limits, rollback/upgrade 안내, version/channel 호환성 정책이 포함된다.
- post-release validation 결과는 GitHub Release 본문 또는 `validation-report.json`에 첨부한다.
...
2026-05-29 완료:
- 배포 후 검증을 artifact 다운로드, checksum, clean install, commandlet post-install smoke, update/uninstall, release notes 제한 사항 확인 기준으로 정의했다.
