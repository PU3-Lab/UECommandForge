...
**아키텍처:** `FMapActorPlacer`가 맵 경로를 검증한 뒤 기존 맵은 `UEditorLoadingAndSavingUtils::LoadMap`으로 열고, 맵이 없으면 `UEditorLoadingAndSavingUtils::NewBlankMap`과 `SaveMap`으로 생성한다. 이후 기존 동일 Blueprint Actor를 제거하고 `GEditor->AddActor`로 새 Actor를 배치한 뒤 맵을 저장한다. `PlaceActorCommandlet`이 Placer를 호출하고 결과를 Result JSON에 기록한다.

**2026-05-27 구현 결과:** Phase 6 완료. `MapActorPlacer`, `PlaceActorCommandlet`, `tools/ue/place_actor.sh`가 추가되었고, `PlaceActor_20260527T122059Z.json` 기준 `ok: true`, `actor_placed: ok`를 확인했다. 반복 실행 시 동일 Actor가 중복 배치되지 않도록 `MapActorPlacer`가 기존 동일 클래스 Actor를 제거한 뒤 다시 배치한다.
...
| 경로 | 책임 |
|---|---|
| `sample/Plugins/UECommandForge/Source/UECommandForgeEditor/Public/Builders/MapActorPlacer.h` | 맵 열기·Actor 배치·저장 인터페이스 |
| `sample/Plugins/UECommandForge/Source/UECommandForgeEditor/Private/Builders/MapActorPlacer.cpp` | 구현 |
| `sample/Plugins/UECommandForge/Source/UECommandForgeEditor/Private/Commandlets/PlaceActorCommandlet.h/.cpp` | 배치 Commandlet |
| `sample/Plugins/UECommandForge/Source/UECommandForgeEditor/Tests/MapActorPlacerTest.cpp` | 자동화 테스트 |
...
- [x] **Step 4: `MapActorPlacer.cpp` 작성**
...
- [x] **Step 1: `MapActorPlacer.h`에 검증 메서드 추가**
...
- [x] **Step 2: `MapActorPlacer.cpp`에 구현 추가**
...
- [x] **Step 1: `PlaceActorCommandlet.h` 작성**
...
- [x] **Step 2: `PlaceActorCommandlet.cpp` 작성**
...
- [x] **Step 3: `tools/ue/place_actor.sh` 작성**
