...
```powershell
$d = "Saved\CodexReports\asset_change_feature_fixtures"
...
```powershell
$plan = Resolve-Path "Saved\CodexReports\asset_change_feature_fixtures\asset_changes.plan.json"
...
```powershell
$d = "Saved\CodexReports\cpp_generation_feature_fixtures"
New-Item -ItemType Directory -Force $d | Out-Null
...
```bat
%UECF_TOOLS%\generate_cpp_class.bat Saved\CodexReports\cpp_generation_feature_fixtures\cpp_health_component.json
```

파일 생성까지 확인하려면 `-Apply`를 붙여 실행한다.

```bat
%UECF_TOOLS%\generate_cpp_class.bat Saved\CodexReports\cpp_generation_feature_fixtures\cpp_health_component.json -Apply
```
...
```powershell
$d = Resolve-Path "Saved\CodexReports\prototype_automation_feature_fixtures"
