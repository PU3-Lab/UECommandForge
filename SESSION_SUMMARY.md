...
릴리즈 패키징 스크립트와 smoke를 독립 리뷰 기반으로 반복 수정했다. 이번 세션의 핵심은 `Tools.zip`과 plugin zip의 검증 신뢰도를 높이고, 아직 구현되지 않은 installer 명령이 manifest에 노출되지 않도록 정리한 것이다.
...
| `tools/release/write_manifest.sh` | package 전체 payload checksum 생성, installer command 빈 배열 유지 |
...
- **브랜치:** `main`
- **워킹 트리:** 이번 세션 변경 사항 커밋 예정
- **커밋 메시지 후보:** `fix: harden release package verification`
